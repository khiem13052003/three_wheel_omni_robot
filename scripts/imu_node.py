...
class BMX160Node:
    def __init__(self):
        rospy.init_node('bmx160_node')

        # Tham s? ROS
        bus = rospy.get_param('~i2c_bus', 1)          # bus I2C, thu?ng 1 ? /dev/i2c-1
        addr = rospy.get_param('~i2c_addr', 0x68)     # d?a ch? I2C n?u khác
        self.frame_id = rospy.get_param('~frame_id', 'imu_link')
        rate_hz = 100        # t?n s? publish, ví d? 50Hz; BMX160 có th? cao hon
        # Covariance (n?u mu?n override)
        cov_acc = rospy.get_param('~cov_accel', 0.02)
        cov_gyro = rospy.get_param('~cov_gyro', 0.02)
        cov_mag = rospy.get_param('~cov_mag', 0.02)
        ...
        try:
            ...
            while not self.bmx.begin():
                rospy.logwarn("BMX160 chua s?n sàng, th? l?i sau 2s")
                time.sleep(2)
        except Exception as e:
            rospy.logerr("Không kh?i du?c BMX160: %s", e)
            rospy.signal_shutdown("Không d?c IMU")
        ...
        # Scale: get_all_data() tr? v? [mx, my, mz, gx, gy, gz, ax, ay, az]
        self.accel_scale = 1.0   
        self.gyro_scale = math.pi/180.0  
        self.mag_scale = 1e-6    # µT ? T (sensor_msgs/MagneticField dùng SI: Tesla)

        # Covariance m?ng 3x3 flattened
        self.cov_accel = [cov_acc, 0,0, 0,cov_acc,0, 0,0,cov_acc]
        self.cov_gyro = [cov_gyro, 0,0, 0,cov_gyro,0, 0,0,cov_gyro]
        self.cov_mag =   [cov_mag,   0,0, 0,cov_mag,  0, 0,0,cov_mag]

        # Timer d? d?c và publish
        self.timer = rospy.Timer(rospy.Duration(1.0/rate_hz), self.timer_cb)

    def timer_cb(self, event):
        try:
            data = self.bmx.get_all_data()
        except Exception as e:
            rospy.logwarn("Ð?c BMX160 l?i: %s", e)
            return

        # unpack
        mx, my, mz = data[0], data[1], data[2]
        gx, gy, gz = data[3], data[4], data[5]
        ax, ay, az = data[6], data[7], data[8]

        now = rospy.Time.now()
        # Publish Imu (gyro + accel). Orientation unknown ? covariance[0] = -1
        imu_msg = Imu()
        imu_msg.header = Header(stamp=now, frame_id=self.frame_id)
        # Không có orientation estimate
        imu_msg.orientation_covariance[0] = -1
        # linear_acceleration
        imu_msg.linear_acceleration.x = ax * self.accel_scale
        imu_msg.linear_acceleration.y = ay * self.accel_scale
        imu_msg.linear_acceleration.z = az * self.accel_scale
        imu_msg.linear_acceleration_covariance = self.cov_accel
        # angular_velocity
        imu_msg.angular_velocity.x = gx * self.gyro_scale
        imu_msg.angular_velocity.y = gy * self.gyro_scale
        imu_msg.angular_velocity.z = gz * self.gyro_scale
        imu_msg.angular_velocity_covariance = self.cov_gyro
        self.pub_imu.publish(imu_msg)

        # Publish MagneticField
        mag_msg = MagneticField()
        mag_msg.header = Header(stamp=now, frame_id=self.frame_id)
        mag_msg.magnetic_field.x = mx * self.mag_scale
        mag_msg.magnetic_field.y = my * self.mag_scale
        mag_msg.magnetic_field.z = mz * self.mag_scale
        mag_msg.magnetic_field_covariance = self.cov_mag
